...
const ContentProduction = ({contentProduction}) => {

  const subServices=contentProduction && contentProduction.attributes.sub_services.data.map((sub)=>{
    return sub
  })
    ...
    <div className="mt-16  font-inter">
      <div className="grid grid-cols-1 md:grid-cols-2 justify-items-center gap-8">
        <div className="grid h-min max-w-lg">
          ...
          <h5 className="text-black text-5xl py-8 font-bold">{contentProduction?.attributes?.name} </h5>
          <div className="grid gap-y-6  h-min">
            <p className="text-gray-600 h-min ">
              {contentProduction?.attributes?.description}
            </p>
